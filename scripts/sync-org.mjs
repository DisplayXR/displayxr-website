...
// Fallback grouping, used only if the mirror has no extensions.json yet.
function extGroup(name) {
  if (/window_binding|surface_binding|gl_binding/.test(name)) return "windowing";
  if (/workspace|app_launcher|file_dialog/.test(name)) return "workspace";
  if (/mcp/.test(name)) return "agent";
  if (/capture/.test(name)) return "capture";
  if (/view_rig|zone|weave|depth_budget/.test(name)) return "rendering";
  return "display";
}

// The mirror publishes extensions.json — name, group, title, summary,
// specVersion, header, specUrl — generated by the runtime's
// scripts/gen_extensions_index.py from the headers joined to its hand-written
// docs/specs/extensions/index.json. Prefer it: the header-filename scan below
// can only guess the group, and cannot tell an extension header from a shared
// one (it used to list XR_DXR_result_codes, which is not an extension).
async function buildExtensions() {
  try {
    const published = await rawJson("displayxr-extensions", "extensions.json");
    if (Array.isArray(published) && published.length) {
      return published
        .map((e) => ({
          ...
          summary: e.summary ?? "",
          group: e.group ?? extGroup(e.name),
          specVersion: e.specVersion ?? null,
          header: e.header ?? `include/openxr/${e.name}.h`,
          specUrl: e.specUrl ?? null,
        }))
        .sort(sortByName);
    }
    warn("displayxr-extensions/extensions.json empty — falling back to header scan");
  } catch (e) {
    warn(`displayxr-extensions/extensions.json — ${e.message} (falling back to header scan)`);
  }
  ...
  const names = paths
    ...
    .filter((n) => n !== "XR_DXR_result_codes");
